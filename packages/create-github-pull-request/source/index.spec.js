...
test.beforeEach(t => {
	...
	github.repos.get = sinon.stub(github.repos, 'get')
		.returnsPromise().resolves({default_branch: 'master'});
	github.repos.getShaOfCommitRef = sinon.stub(github.repos, 'getShaOfCommitRef')
		.returnsPromise().resolves({sha: 'abcd1234'});
	github.gitdata.createReference = sinon.stub(github.gitdata, 'createReference')
		.returnsPromise().resolves({});
	github.gitdata.getTree = sinon.stub(github.gitdata, 'getTree')
		.returnsPromise().resolves({tree: [
			{path: '.babelrc', sha: '1234abcd'},
			{path: 'package.json', sha: 'abcd0987'}
		]});
	github.repos.updateFile = sinon.stub(github.repos, 'updateFile')
		.returnsPromise().resolves({});

	github.pullRequests.create = sinon.stub(github.pullRequests, 'create')
		.returnsPromise().resolves({});
...
test('when called with invalid parameters', t => {
	t.throws(createGithubPullRequest(), AssertionError,
		'does not allow empty parameters');

	t.throws(createGithubPullRequest({}), AssertionError,
		'does not allow object as first parameter');

	t.throws(createGithubPullRequest('owner/repo/x', 'content'), AssertionError,
...
test('committing the updated file', async t => {
	...
	await createGithubPullRequest(uri, content, {client});
	{
		...
		t.is(actual, expected, 'should specify the path to the file being updated');
	}
	{
	...
	}
	{
		...
		t.is(actual, expected, 'should create commit on previously created branch');
	}
});
...
test('creating a pull request', async t => {
	...
	}
	{
		...
		t.is(actual, expected, 'should point the HEAD to the newly created branch');
	}
	{
...
test('updating a file in a sub-directory', async t => {
	...
	client.gitdata.getTree = sinon.stub(client.gitdata, 'getTree')
		.returnsPromise().resolves({tree: [
...

	{
		...
		t.is(actual, expected, 'should get the tree for the parent path of file');
	}
	{
		const actual = client.gitdata.createReference.firstCall.args[0].ref;
...
test('overriding `options.baseBranch`', async t => {
	...
	await createGithubPullRequest(uri, content, {client, baseBranch: 'new_base'});

	{
		const actual = client.repos.getShaOfCommitRef.firstCall.args[0].ref;
		const expected = 'new_base';

		t.is(actual, expected, 'should get the HEAD of the specified baseBranch');
	}
	{
		const actual = client.pullRequests.create.firstCall.args[0].base;
		const expected = 'new_base';

		t.is(actual, expected, 'should point the base to the specified baseBranch');
	}
});
...
test('overriding `options.message`', async t => {
	const client = t.context.github;
	const message = 'this is the title\n\nthis is the body\n\nthis is the footer';
