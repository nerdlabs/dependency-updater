...
test('when called with invalid parameters', t => {
	t.throws(getGithubFile(), AssertionError,
...
test('when passing in a url to github repo', async t => {
...

	{
		...
		t.deepEqual(actual, expected, 'it should parse user and repo from the uri');
	}
	{
...
test('when fetching a file from an unauthenticated repository', async t => {
	...
	const actual = await getGithubFile('user/repo/file.txt', {client: github});
